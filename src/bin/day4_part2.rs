...
fn line_points(mut line: &[u8]) -> usize {
...
fn calc(input: impl Iterator<Item = impl AsRef<str>>) -> usize {
    let mut sum: usize = 0;
    let mut lines: VecDeque<usize> = VecDeque::new();
    for line in input {
        let instance_count = lines.pop_front().unwrap_or_default() + 1;
        let line = line.as_ref().as_bytes();
        let points = line_points(line);
        for i in 0..points {
            if let Some(i) = lines.get_mut(i) {
                *i += instance_count;
            } else {
                lines.push_back(instance_count);
            }
        }
        sum += instance_count;
    }
